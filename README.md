...
Mapbox GL で地図を表示するには、ランタイム用のアクセストークンも必要です。
`mobile` ディレクトリに `.env` ファイルを作成し、次のように設定してください。

```bash
MAPBOX_ACCESS_TOKEN=<表示用トークン>
```

サンプルとして `mobile/.env.example` を用意してあります。必要に応じてコピーして
使用してください。
